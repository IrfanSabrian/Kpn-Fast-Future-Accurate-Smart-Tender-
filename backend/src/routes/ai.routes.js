...
router.post('/scan-document', upload.single('file'), async (req, res) => {
  try {
    ...
    console.log(`[AI SCAN] Received request to scan ${documentType.toUpperCase()}, file size: ${(req.file.size / 1024).toFixed(2)}KB`);

    // Scan document with Gemini AI
    const extractedData = await geminiAIService.scanDocument(req.file.buffer, documentType);
