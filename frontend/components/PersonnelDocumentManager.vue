<template>
  <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
    ...
    <div v-if="!hasDocument" class="flex flex-col items-center justify-center py-12">
      <div class="w-16 h-16 rounded-full flex items-center justify-center mb-4" :class=icon BackgroundClass>
        <i :class="`${iconClass} text-2xl`"></i>
      </div>
      <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-1">{{ documentLabel }} Belum Tersedia</h3>
      <p class="text-sm text-slate-500 mb-6">Belum ada data {{ documentLabel }} untuk personel ini</p>
      <button
        @click="$emit('add')"
    ...
    <div v-else class="p-6">
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ documentLabel }}</h3>
