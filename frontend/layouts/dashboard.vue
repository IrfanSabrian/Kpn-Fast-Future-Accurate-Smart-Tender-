<template>
  <div
    class="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300 font-sans"
  >
    ...
    <button
      @click="toggleSidebar"
      class="fixed top-4 left-4 z-30 lg:hidden w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
      :class="{ 'left-[296px]': isSidebarOpen }"
    >
      <i
        class="fas text-lg transition-transform duration-300"
        :class="isSidebarOpen ? 'fa-times rotate-90' : 'fa-bars'"
      ></i>
    </button>

    <!-- ENGINEERING SIDEBAR (Control Panel) -->
    <aside
      class="fixed left-0 top-0 z-30 h-screen w-[280px] bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col text-slate-600 dark:text-slate-300 shadow-xl transition-all duration-300"
      :class="{
    ...
    >
      <!-- Brand Header -->
      <div
        class="h-24 flex items-center px-6 border-b border-slate-100 dark:border-white/5 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm"
      >
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 flex-shrink-0 bg-white dark:bg-slate-800 rounded-lg p-1 border border-slate-100 dark:border-slate-700 shadow-sm"
          >
            <img
              src="/Logo.png"
              alt="KPN Logo"
          ...
          <div>
            <h1
              class="text-lg font-black text-slate-800 dark:text-white tracking-tight leading-none mb-1"
            >
              KPN FAST
            </h1>
          </div>
      ...
      <nav class="flex-1 overflow-y-auto px-4 py-8 space-y-8 custom-scrollbar">
        ...
        <div>
          <h3
            class="text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-4 px-3"
          >
            General
          </h3>
          <div class="space-y-1">
            ...
            >
              <i
                class="fas fa-th-large w-5 text-center group-hover:text-blue-500 transition-colors"
              ></i>
              <span>Dashboard</span>
            </NuxtLink>
            <div
              class="opacity-50 pointer-events-none flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium"
            >
              <i class="fas fa-chart-pie w-5 text-center"></i>
              <span>Analytics</span>
              <span
                class="ml-auto text-[9px] bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-500 font-mono"
                >SOON</span
              >
            </div>
          </div>
        ...
        <div>
          ...
          <div class="space-y-1">
            ...
            >
              <i
                class="fas fa-building w-5 text-center group-hover:text-blue-500 transition-colors"
              ></i>
              <span>Perusahaan</span>
            </NuxtLink>
            <NuxtLink
              to="/database/personel"
              @click="handleNavClick"
              class="flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-bold transition-all group hover:bg-slate-50 dark:hover:bg-slate-800"
              active-class="bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/20 shadow-sm"
            >
              <i
                class="fas fa-users w-5 text-center group-hover:text-blue-500 transition-colors"
              ></i>
              <span>Personel</span>
            </NuxtLink>
            <div
      ...
      >
        ...
        >
          <div
            class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center overflow-hidden border-2 border-white dark:border-slate-700 shadow-lg relative shrink-0"
          >
            <!-- Loading State -->
            <div
              v-if="loadingProfile"
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              ...
              <span
                class="hidden sm:inline text-[9px] bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded font-mono border border-blue-100 dark:border-blue-800"
              >
                {{ userProfile.role }}
              </span>
            </div>
          ...
          <button
            ...
            class="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition-colors shrink-0"
          >
    ...
    <!-- Main Content Area -->
    <div
      class="flex-1 flex flex-col min-h-screen transition-all duration-300 lg:ml-[280px]"
    >
      <main class="flex-1 relative pt-20 lg:pt-0">
        <!-- Optional top fade for seamless scroll feeling -->
        <div
          class="absolute top-0 left-0 w-full h-8 bg-gradient-to-b from-slate-50 to-transparent dark:from-slate-950 z-10 pointer-events-none"
        ></div>
        <slot />
      </main>
    </div>
    ...
    >
      <div
        class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-slate-200 dark:border-slate-800"
        @click.stop
      >
        <div
        ...
        >
          <i
            class="fas fa-sign-out-alt text-red-600 dark:text-red-400 text-2xl"
          ></i>
        </div>

        <h3
          class="text-xl font-bold text-slate-800 dark:text-white text-center mb-2"
        >
          Logout Akun Google?
        </h3>
        <p class="text-sm text-slate-500 dark:text-slate-400 text-center mb-6">
          Anda akan keluar dari akun Google dan perlu login ulang untuk
          mengakses fitur cloud.
        </p>
